...
import { Requestor } from "../../src/request-imp/request-xhr-imp";

const requestor = new Requestor("http://localhost:3000");

describe("测试 XMLHttpRequest 封装的Requestor基本功能", () => {
	const methods = ["get", "post", "head", "patch", "put", "delete", "options"] as const;
	for (const method of methods) {
		it(`测试 ${method} 普通请求`, async () => {
			const resp = await requestor[method](`/api/${method}`);
...
describe("测试 XMLHttpRequest 封装的Requestor options功能", () => {
	it("测试携带 请求头", async () => {
	...
	it("测试携带 格式为 FormData 的 请求体", async () => {
		...
		body.append("password", "123456"); // 携带 FormData 格式的 请求体响应成功
...
describe("测试 XMLHttpRequest 封装的Requestor 失败功能", () => {
	it("测试 client 401 错误是否能被捕获", () => {
		requestor.get("/api/client/error").catch(resp => {
			expect(resp.ok).toBe(false);
			expect(resp.status).toBe(401);
			expect(resp.json()).resolves.toEqual({
